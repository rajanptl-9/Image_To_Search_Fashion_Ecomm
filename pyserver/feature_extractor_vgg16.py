...
from tensorflow.keras.applications import VGG16  # Changed from ResNet50 to VGG16
from tensorflow.keras.applications.vgg16 import preprocess_input  # Changed to VGG16 preprocess
...
class FeatureExtractor:
    def __init__(self):
        ...
        self.model = VGG16(weights='imagenet', include_top=False, pooling='avg')  # Changed to VGG16
    
    def extract_features(self, img_path):
        # Load and preprocess image
        img = image.load_img(img_path, target_size=(224, 224))  # VGG16 also uses 224x224
        x = image.img_to_array(img)
        x = np.expand_dims(x, axis=0)
        x = preprocess_input(x)  # This now uses VGG16's preprocess_input
